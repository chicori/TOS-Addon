CHAT_SYSTEM("MARKET SHOW LEVEL JP v1.0.5 Ex loaded!");
	...
	for i = 0 , count - 1 do
		...
		if itemGroup == "Weapon" or itemGroup == "SubWeapon" then
			...
			if socketInfo == "[0]" then socketInfo = "" end;
						name:SetTextByKey("value", GET_FULL_NAME(itemObj)  .. socketInfo .. "{nl}" .. gemInfo .. "" .. "PR:" .. itemObj["PR"]);
		...
		elseif itemGroup == "Armor" then
			...
			local socketInfo = "[" .. itemObj["MaxSocket"] .. "]";
			if socketInfo == "[0]" then socketInfo = "" end;
			name:SetTextByKey("value", GET_FULL_NAME(itemObj) .. socketInfo .. prop .. gemInfo .. "" .. "PR:" .. itemObj["PR"]);
		...
		elseif itemGroup == "Gem" then
			...
			gemSkill = string.gsub(gemSkill,"Weapon","武器");
			gemSkill = string.gsub(gemSkill,"Foot","足");
			gemSkill = string.gsub(gemSkill,"Hand","手");
			gemSkill = string.gsub(gemSkill,"TopLeg","体");
			if itemObj["EquipXpGroup"] == "Gem" then gemSkill = "" end;
